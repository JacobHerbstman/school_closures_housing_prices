...
Four samples: all property and single-family, each within a quarter mile and a
half mile. A sale is treated (control) when a closed (stayed-open) site is
within the radius and no site of the other group is; sales within the radius
of a welcoming school, another February candidate, or a building vacated by a
...
2008--2012 and omits 2013. Three control sets:

1. site and year fixed effects only;
2. hedonics: log building and lot area and age (each with its square), beds,
   rooms, full baths, residence type, construction quality, repair condition,
   REO and quick-resale indicators, and a two-to-six-unit indicator;
3. the same, with the unit indicator replaced by a unit-count factor that has
   an explicit unknown level.

Property-class dummies are not used: classes 205/207 and 210/295 change at age
62. In the single-family samples sets 2 and 3 coincide. `output/twfe.rds` holds
