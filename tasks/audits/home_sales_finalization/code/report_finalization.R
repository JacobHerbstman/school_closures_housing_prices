...
lines <- c(
  ...
  "## Property corrections and sample definition", "",
  "Home-improvement exemption (HIE) records update property characteristics before sales are selected for the price analysis. Production preserves every master transaction, the original sale class, the original improvement fields, and source exemption IDs. Uncertain replacement fields stay missing; a missing correction never silently restores an older value.", "",
  "The analysis class comes from the corrected same-year improvement record. Single-family classes paired with observed multiple apartments or multifamily use, and class 211 paired with no apartments or single-family use, receive a missing analysis class. These conflicts stay in the master but not the complete-hedonic price sample. Missing use alone is not an extra complete-case restriction. Missing apartment count still excludes class 211. Class 212 stays outside the price sample. After these screens and the existing integrity exclusions, the final sample removes nominal price per building square foot strictly above the citywide 99.9th percentile within sale year (R quantile type 7). The historical comparison samples do not receive this new trim.", "",
  markdown_table(audit$price_tail_summary),
  ...
  "## Sample changes and the choice of property classification", "",
  "The previous sample uses the uncorrected property characteristics and sale-record property class, without percentile trimming. The final sample uses exemption-corrected characteristics, resolves property class against recorded use and apartment count, and applies the annual price-per-square-foot trim.", "",
  ...
  "The broad geocoded 2006–2025 master is independent of all analytical sample restrictions. The school-distance and exposure tasks use this master to assign proximity to the supplied school locations. This housing audit checks coordinates before those geographic sample restrictions.", "",
  ...
  "Every supported corrected field agrees with the rule-based home-improvement exemption audit on every master row (tasks/audits/home_improvement_exemptions/code/apply_careful_corrections.R). Every original field is preserved, all non-corrected baseline fields are unchanged, all correction links fall within their active-year intervals, selected IDs independently reproduce the cleaner, and geocoding preserves every retained sale field.", "",
  "## Primary sources", "",
