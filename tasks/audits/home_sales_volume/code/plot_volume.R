...
annual <- copy(volume$annual)[, `:=`(group = fifelse(treated == 1L, "Closed", "Stayed open"),
...
events <- rbind(volume$events[, .(property, measure, sale_year, estimate, ci_low, ci_high)],
                unique(volume$events[, .(property, measure)])[, `:=`(sale_year = 2012L, estimate = 0, ci_low = 0, ci_high = 0)])
...
did <- copy(volume$did)[, measure := factor(measure, levels = rev(measure_levels))]
